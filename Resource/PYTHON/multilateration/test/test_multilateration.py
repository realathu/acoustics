...
def calculate_toa_array():  
    ...
    dist_src_starboard = float(math.sqrt(math.pow(param.TestParameters.SOURCE_POS_X - param.HydrophoneDetails.STARBOARD_HYD_X, 2) + 
                                      ...
                                      math.pow(param.TestParameters.SOURCE_POS_Z - param.HydrophoneDetails.STARBOARD_HYD_Z, 2)))
    ...
    samp_port = np.int32(param.DSPConstants.SAMPLE_FREQUENCY *(dist_src_port / param.PhysicalConstants.SOUND_SPEED)) # 
    samp_starboard = np.int32(param.DSPConstants.SAMPLE_FREQUENCY *(dist_src_starboard / param.PhysicalConstants.SOUND_SPEED)) # 
    samp_stern = np.int32(param.DSPConstants.SAMPLE_FREQUENCY*(dist_src_stern / param.PhysicalConstants.SOUND_SPEED)) #* 
