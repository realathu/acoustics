...
class DSPConstants:
    ...
    # per hydrophone
    DMA_BUFFER_LENGTH: int = (
        IN_BUFFER_LENGTH / 2
    )  # Number real measurements transferred with DMA
...
class HydrophoneDetails:
    ...
    PORT_HYD_Z: float = 0.0  # z - position of port hydrophone [m]
    ...
    STARBOARD_HYD_Z: float = 0.0  # z - position of starboard hydrophone [m]

    STERN_HYD_X: float = 0.0  # x - position of stern hydrophone [m]
    STERN_HYD_Y: float = -0.24  # y - position of stern hydrophone [m]
    STERN_HYD_Z: float = 0.0  # z - position of stern hydrophone [m]
...
class FilterSetup:
    NUM_STAGES: int = 2
    # Number of second order cascade-filters. Determinse the
    # filter order. Order = 2 * num_stages
